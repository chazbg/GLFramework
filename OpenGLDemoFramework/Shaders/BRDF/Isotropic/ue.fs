...
// vec3 getSpecularContribution(vec3 n, vec3 v, vec3 l, float m, float ior)
// {
    // vec3 h = normalize(v + l);
    // float NoH = max(0.0, dot(n, h));
    // //return schlick(vec3(ior), v, h) * (m + 8) * pow(NoH, m);
    // return schlick(vec3(0.96, 0.64, 0.54), v, h) * (m + 8) * pow(NoH, m);
// }

float getSpecularContribution(vec3 n, vec3 v, vec3 l, float m, float ior)
{
    float res = 0;
    float aSq = pow((1-glossiness),2);
    vec3 h = normalize(v + l);
    float NoH = max(0.0, dot(n, h));
    float NoV = max(0.0, dot(n, v));
    float NoL = max(0.0, dot(n, l));
    float D = aSq * INVERSE_PI / pow((pow(NoH, 2) * (aSq - 1) + 1), 2);
    //float D = (m + 8) * pow(NoH, m);
    //return schlick(vec3(ior), v, h) * D;
    if (NoL > 0)
    {
        res = schlick(smoothstep(0.2, 0.99, ior), v, h) * D * G_Smith(v, n, l, 1-glossiness) / (NoH * NoV);
    }
    return res;
}
...
vec3 getEnvMapContribution(vec3 n, vec3 v, vec3 r, float m, float ior)
{
    ...
    vec3 envCol = sRGBToLinear(textureLod(envMap, r, (1 - glossiness) * 8).bgr); 
    //vec3 envCol = sRGBToLinear(texture(envMap, r).bgr); 
    return getSpecularContribution(n, v, r, m, ior) * envCol;
}
...
void main()
{
    ...
    specularContribution += getSpecularContribution(n, v, light0.L, m, ior) * light0.iL * NoL0 * vec3(1);
    specularContribution += getSpecularContribution(n, v, light1.L, m, ior) * light1.iL * NoL1 * vec3(1);
    specularContribution += getSpecularContribution(n, v, light2.L, m, ior) * light2.iL * NoL2 * vec3(1);
    specularContribution += getEnvMapContribution(n, v, r, m, ior);   

    //vec3 f0 = vec3(0.96, 0.64, 0.54);//copper
    vec3 f0 = vec3(1, 0.71, 0.29);//gold                                            // * 0.125 * INVERSE_PI
    vec3 result = blendMaterial(f0 * diffuseContribution * INVERSE_PI, specularContribution, f0, ior);
